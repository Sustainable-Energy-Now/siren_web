...
class Optimisation:
    def __init__(self, name, approach, values): #capacity=None, cap_min=None, cap_max=None, cap_step=None, caps=None):
        ...
        if approach == 'Discrete':
            ...
            caps = values.split()
            try:
                self.capacity_min = caps[0]
            except:
                self.capacity_min = 0.
            try:
                self.capacity_max = caps[1]
            except:
                self.capacity_max = 0.
            try:
                self.capacity_step = caps[2]
            except:
...
class powerMatch():
    def __init__ (self, config, constraints=dict, generators=dict,):
    ...
    @staticmethod
    def doDispatch(config, year, option, pmss_details, pmss_data, generators, re_order, dispatch_order,
    ...
    # Note: For Batch pmss_data is reused so don't update it in doDispatch
        ...
        carbon_price = float(config.get('Powermatch','carbon_price'))
        discount_rate = float(config.get('Powermatch','discount_rate'))
        adjusted_lcoe = True
        optimise_debug = False
        scenarios = 'C:/Users/Paul/Local Sites/Powermatch/'
        results_prefix = ''
        save_tables = config.get('Powermatch','save_tables')
...
#            ns.cell(row=max_row, column=col-1).number_format = '#,##0.00'
        ...
        if option != 'D':
            ...
            for sp in range(len(sp_data)):
                ...
                if not generators[gen].fuel:
                    generators[gen].fuel = 0
                if generators[gen].capex > 0 or generators[gen].fixed_om > 0 \
                  or generators[gen].variable_om > 0 or generators[gen].fuel > 0:
                    if remove_cost and sp_data[sp][ndx] == 0:
                        sp_data[sp][st_cst] = 0
                        continue
                    ...
                    disc_rate = generators[gen].disc_rate
                    if disc_rate == 0:
                        disc_rate = float(config.get('Powermatch','discount_rate'))
                ...
                if generators[gen].area > 0:
                    sp_data[sp][st_are] = sp_data[sp][st_cap] * generators[gen].area
            ...
            if do_underlying:
                ...
                for fac in underlying_facs:
                    ...
                    if generators[gen].area > 0:
                        sp_d[st_are] = sp_d[st_cap] * generators[gen].area
                    sp_data.append(sp_d)
                if gen_sum > 0:
                    gs = cost_sum / gen_sum
                else:
    ...
    @staticmethod
    def optClicked(config, in_year, in_option, in_pmss_details, in_pmss_data, generators, in_re_order,
        ...
        def calculate_fitness(population):
            ...
            for chromosome in population:
                ...
                multi_value, op_data, extra = powerMatch.doDispatch(
                    config, year, option, pmss_details, pmss_data, generators, re_order,
