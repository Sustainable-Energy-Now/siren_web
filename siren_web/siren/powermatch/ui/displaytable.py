...
from utilities.senutils import ssCol, techClean
...
#       somewhere we need to cater for no fields and a sumby
        ...
        for cl in range(self.table.columnCount()):
            self.table.horizontalHeaderItem(cl).setIcon(QtGui.QIcon('resources/blank.png'))
    ...
    def order(self, col):
        if col < 0:   # key == self.name:
            ...
            if col != self.sort_col:
                self.table.horizontalHeaderItem(self.sort_col).setIcon(QtGui.QIcon('resources/blank.png'))
                self.sort_asc = False
            self.sort_col = col
            ...
            if self.sort_asc:   # swap order
                ...
                for key, value in iter(sorted(norderd.items())):
                    torder.append(value)
                self.table.horizontalHeaderItem(col).setIcon(QtGui.QIcon('resources/arrowd.png'))
                self.sort_asc = False
            else:
                ...
                for key, value in iter(sorted(orderd.items())):
                    torder.append(value)
                self.table.horizontalHeaderItem(col).setIcon(QtGui.QIcon('resources/arrowu.png'))
                self.sort_asc = True
