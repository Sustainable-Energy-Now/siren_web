...
import shutil
from api.api import ApplicationAPI
import subprocess
import sys
import time
import webbrowser
...
from siren.ui.editini import EdtDialog, EditFileSections
from modules.getmodels import getModelFile, commonprefix
from utilities.senutils import ClickableQLabel, getUser
...
class TabDialog(QtWidgets.QDialog):
    ...
    def setup_ui(self, parent=None):
        ...
        utilicon = ['resources/line.png', 'resources/download.png', 'resources/map.png', 'resources/download.png', 'resources/weather.png', 'resources/line.png', 'resources/download.png']
        spawns = []
        icons = []
        if sys.platform == 'win32' or sys.platform == 'cygwin':
        ...
        self.model_icon = ['resources/line.png', 'resources/map.png', 'resources/list.png', 'resources/grid.png', 'resources/sen_icon32.png',
                           'resources/power.png', 'resources/line.png', 'resources/list.png']
        ...
        if self.table.selectedIndexes() != []:
            ...
            if (event.type() == QtCore.QEvent.MouseButtonPress or event.type() == QtCore.QEvent.MouseButtonRelease) and \
              event.button() == QtCore.Qt.RightButton:
                ...
                actions =  []
                if ent == 'getfiles.ini':
                    actions.append(menu.addAction(QtGui.QIcon('resources/edit.png'), 'Edit Preferences'))
                    ...
                    actions.append(menu.addAction(QtGui.QIcon('resources/edit.png'), 'Edit Preferences'))
                    actions[-1].setIconVisibleInMenu(True)
                    actions.append(menu.addAction(QtGui.QIcon('resources/edit.png'), 'Edit File Preferences'))
                    actions[-1].setIconVisibleInMenu(True)
                    actions.append(menu.addAction(QtGui.QIcon('resources/copy.png'), 'Copy Preferences'))
                    actions[-1].setIconVisibleInMenu(True)
                    actions.append(menu.addAction(QtGui.QIcon('resources/delete.png'), 'Delete Preferences'))
                    actions[-1].setIconVisibleInMenu(True)
    ...
    def invoke(self, program, ent):
        """
        Invoke a program with an entity parameter, supporting both executables and Python scripts.
        Args:
            program: Base name of the program (without extension)
            ent: Entity parameter to pass to the program
        Returns:
        ...
        try:
            if sys.platform in ('win32', 'cygwin'):
                ...
                exe_path = f"{program}.exe"
                py_path = f"{program}/main.py"
                ...
                if os.path.exists(exe_path):
                    ...
                    process = subprocess.Popen(
                        ['python', py_path, str(ent)],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
                        universal_newlines=True
                    )
                else:
                    raise FileNotFoundError(f"Neither {exe_path} nor {py_path} found")
            else:  # Linux/Mac
                py_path = f"{program}.py"
                if not os.path.exists(py_path):
                    raise FileNotFoundError(f"Script not found: {py_path}")
                    
                process = subprocess.Popen(
                    ['python3', py_path, str(ent)],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    universal_newlines=True
                )
            ...
            print(f"Error invoking program: {str(e)}")
            raise
    ...
    def spawn(self, who):
        """
        Spawn a subprocess with error handling.
        Args:
            who: Either a list containing [script_path, *args] or a single script path
        Returns:
            subprocess.Popen: The spawned process object
        """
        try:
            # Convert single string to list for consistent handling
            cmd_list = who if isinstance(who, list) else [who]
            script_path = cmd_list[0]
            
            if not os.path.exists(script_path):
                raise FileNotFoundError(f"Script not found: {script_path}")
            
            # Build command list based on platform and file type
            if sys.platform in ('win32', 'cygwin'):
                if script_path.endswith('.py'):
                    cmd = ['python', script_path]
                    if len(cmd_list) > 1:
                        cmd.extend(cmd_list[1:])
                    # Don't use shell=True for security and debugging
                    process = subprocess.Popen(
                        cmd,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
            ...
            print(f"Error spawning subprocess: {str(e)}")
            raise
...
class makeNew(QtWidgets.QDialog):
    ...
    def __init__(self, models_dirs=None, help='help.html'):
        ...
        utilicon = ['resources/download.png', 'resources/map.png', 'resources/download.png', 'resources/weather.png', 'resources/download.png']
        spawns = []
        icons = []
        if sys.platform == 'win32' or sys.platform == 'cygwin':
            for i in range(len(utilities)):
...
class ApplicationUI:
    """UI layer that handles user interaction"""
    def __init__(self):
    ...
    def setup(self):
        ...
        self.api.initialize()
        self.main_window = TabDialog(self.api)
